        ...
        <div className="max-w-3xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="mt-8 bg-white p-8 rounded-2xl shadow-lg space-y-6">
                ...
                <div>
                    ...
                    <Input type="text" name="excerpt" id="excerpt" value={formData.excerpt} onChange={handleChange} required className="mt-1" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label htmlFor="difficulty">Difficulté</label>
                        <Select id="difficulty" name="difficulty" value={formData.difficulty} onChange={handleChange} className="mt-1">
                            {Object.values(TrailDifficulty).map(d => <option key={d} value={d}>{d}</option>)}
                        </Select>
                    </div>
                    <div>
                        <label htmlFor="distanceKm">Distance (km)</label>
                        <Input type="number" step="0.1" name="distanceKm" id="distanceKm" value={formData.distanceKm} onChange={handleChange} required className="mt-1" />
                    </div>
                </div>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div>
                        <label htmlFor="durationMin">Durée (minutes)</label>
                        <Input type="number" name="durationMin" id="durationMin" value={formData.durationMin} onChange={handleChange} required className="mt-1" />
                    </div>
                     <div>
                        <label htmlFor="ascentM">Dénivelé positif (m)</label>
                        <Input type="number" name="ascentM" id="ascentM" value={formData.ascentM} onChange={handleChange} required className="mt-1" />
                    </div>
                 </div>
                <div>
                    <label htmlFor="gpxUrl">URL du fichier GPX (optionnel)</label>
                    <Input type="text" name="gpxUrl" id="gpxUrl" value={formData.gpxUrl} onChange={handleChange} className="mt-1" />
                </div>
                <div>
