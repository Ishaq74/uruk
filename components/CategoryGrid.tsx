...
const CategoryCard: React.FC<{ category: Category, navigateTo: (page: string, id?: string, mainCategory?: Place['mainCategory'], query?: string) => void }> = ({ category, navigateTo }) => {
    ...
    return (
        <a href="#" onClick={(e) => { e.preventDefault(); navigateTo(category.target); }} className="group flex flex-col items-center justify-center p-6 bg-white rounded-xl shadow-md hover:shadow-xl hover:bg-sky-50 transition-all duration-300 transform hover:-translate-y-1">
            ...
            <h4 className="mt-4 font-bold text-lg text-gray-800 group-hover:text-sky-600 transition-colors text-center">{t(category.name)}</h4>
        </a>
    );
}
...
const CategoryGrid: React.FC<CategoryGridProps> = ({ navigateTo }) => {
  ...
  return (
    <section>
      <h2 className="text-3xl font-bold tracking-tight text-center text-gray-900 sm:text-4xl">{t('categories.title')}</h2>
      <p className="mt-4 text-lg text-center text-gray-600 max-w-2xl mx-auto">{t('categories.subtitle')}</p>
